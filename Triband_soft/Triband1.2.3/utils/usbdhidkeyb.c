...
//extern void SendString(uint8_t *pcStr);
//extern int string_parsing(uint8_t *parameter_passed);
uint8_t *recieve_temp;
int packet = 0;
/*char *recieve_new[1856];*/
//*****************************************************************************
//
...
static uint8_t g_pui8KeybDescriptor[] =
{
    ...
    USBShort(41),               // The total size of this full structure.
...
static const tHIDDescriptor g_sKeybHIDDescriptor =
{
    ...
    0x100,                          // bcdHID (version 1.11 compliant)
...
//    int count = 0, first = 0, last = 0;
    ...
    switch (ui32Event)
    {
        ...
        /* if(packet == 0)
         {
        	 recieve_temp =  malloc(64);
         }
         memcpy(&recieve_temp[packet*64],psInst->pui8DataBuffer, 64);
         if(memchr(psInst->pui8DataBuffer, 13, 64)){
        	 parse_string(&recieve_temp[0]);
        	 packet =0;
         }
         else
         {
        	 packet++;
        	 realloc(recieve_temp, (packet+1)*64);
         }
         */
         collect_data_in_buffer(psInst->pui8DataBuffer);
            //
