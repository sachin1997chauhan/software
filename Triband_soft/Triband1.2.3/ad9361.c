...
/* Rx Gain Tables */
#if   (dbm==10)
#define SIZE_FULL_TABLE	    15
#elif (dbm==20)
#define SIZE_FULL_TABLE	    25
#define SIZE_SPLIT_TABLE        25
#elif (dbm==25)
#define SIZE_FULL_TABLE	    33
#elif (dbm==30)
#define SIZE_FULL_TABLE	    35
#elif (dbm==33)
#define SIZE_FULL_TABLE	    38
#elif (dbm==35)
#define SIZE_FULL_TABLE	    41
#elif (dbm==40)
#define SIZE_FULL_TABLE	    46
#elif (dbm==50)
#define SIZE_FULL_TABLE	    60
#elif (dbm==55)
#define SIZE_FULL_TABLE	    70
#else
...
{ {  /* 800 MHz */
	...
	{ 0x64, 0x21, 0x00 }, { 0x64, 0x22, 0x00 }, { 0x64, 0x23, 0x00 },
	{ 0x64, 0x24, 0x00 }, { 0x64, 0x25, 0x00 }, { 0x64, 0x26, 0x00 },
	{ 0x64, 0x27, 0x00 }, { 0x64, 0x28, 0x00 }, { 0x64, 0x29, 0x00 },
	{ 0x64, 0x2A, 0x00 }, { 0x64, 0x2B, 0x00 }, { 0x64, 0x2C, 0x00 },
	{ 0x64, 0x2D, 0x00 }, { 0x64, 0x2E, 0x20 }, { 0x64, 0x2F, 0x00 },
	{ 0x64, 0x30, 0x00 }, { 0x64, 0x31, 0x00 }, { 0x64, 0x32, 0x00 },
	{ 0x64, 0x33, 0x00 }, { 0x64, 0x34, 0x00 }, { 0x64, 0x35, 0x00 },
	{ 0x64, 0x36, 0x00 }, { 0x64, 0x37, 0x00 }, { 0x64, 0x38, 0x00 },
	{ 0x65, 0x38, 0x20 }, { 0x66, 0x38, 0x20 }, { 0x67, 0x38, 0x20 },
...
static const uint8_t split_gain_table[RXGAIN_TBLS_END][SIZE_SPLIT_TABLE][3] =  //(here [RXGAIN_TBLS_END]=3 means total number of tables are 3 )
{ {  /* 800 MHz (table-1) */                                                   //[SIZE_SPLIT_TABLE]=41 means total number of row are 41
	{ 0x00, 0x18, 0x20 }, { 0x00, 0x18, 0x00 }, { 0x00, 0x18, 0x00 },          //[3] it stands for the number of columns
 ...
 */
int32_t ad9361_spi_readm(/*char par*/struct spi_device  *spi,uint32_t reg,	uint8_t *rbuf, uint32_t num)
{
	uint8_t buf[3];
	int32_t ret;
	uint16_t cmd;
	...
	SysCtlDelay(100);
	ret = spi_write_then_read(spi,&buf[0], 2, rbuf, num);// check here what value is coming here in rbuf variable
	if (ret < 0) {
 ...
 */
int32_t ad9361_spi_read(struct spi_device  *spi/*char par*/, uint32_t reg)
{
	uint8_t buf=0;
	int32_t ret=0;
 ...
 */
static int32_t __ad9361_spi_readf(struct spi_device  *spi/*char par*/, uint32_t reg,
 ...
 * @return The bits value or negative error code in case of failure.
 */
#define ad9361_spi_readf(par, reg, mask) \
	__ad9361_spi_readf(par, reg, mask, __ffs(mask))
 ...
 * @return 0 in case of success, negative error code otherwise.
 */
int32_t ad9361_spi_write(/*char par*/struct spi_device  *spi,
	uint32_t reg, uint32_t val)
{
	...
	ret = spi_write_then_read(spi,buf, 3, rxbuf, 0);
	if (ret < 0) {
 ...
 * @return 0 in case of success, negative error code otherwise.
 */
#define ad9361_spi_writef(phy/*par*/, reg, mask, val) \
	__ad9361_spi_writef(phy, reg, mask, __ffs(mask), val)
...
int32_t ad9361_reset(struct ad9361_rf_phy *phy )
{
    ...
    char par=phy->spi->id_no;
    GPIOPinWrite(phy->spi->dev.reset_pin_addr,  phy->spi->dev.reset_pin,  ~(phy->spi->dev.reset_pin));
    SysCtlDelay(2000);
    GPIOPinWrite(phy->spi->dev.reset_pin_addr,  phy->spi->dev.reset_pin,  (phy->spi->dev.reset_pin));
...
/*
static int32_t ad9361_int_loopback_fix_ch_cross(char par,struct ad9361_rf_phy *phy, bool enable)
{
	if (!phy->pdata->rx2tx2 && phy->pdata->rx1tx1_mode_use_rx_num !=
			phy->pdata->rx1tx1_mode_use_tx_num)
		return ad9361_en_dis_tx(par,phy, TX_1 | TX_2,
	...
	uint32_t mask, bool done_state)
{
	...
	uint32_t state;
	/*ret =*/ ad9361_spi_write(phy->spi,REG_CTRL_OUTPUT_POINTER,0x01);
	/*ret = */ad9361_spi_write(phy->spi,REG_CTRL_OUTPUT_ENABLE,0xFF);
	SysCtlDelay(20000);
	//state = ad9361_spi_read(par,REG_CH_1_OVERFLOW);
	/*ret = */ad9361_spi_read(phy->spi,REG_CTRL_OUTPUT_POINTER);
	/*ret = */ad9361_spi_read(phy->spi,REG_CTRL_OUTPUT_ENABLE);
...
//		__func__, freq, band);
	...
	ad9361_spi_writef(phy->spi, REG_AGC_CONFIG_2,
	...
	ad9361_spi_write(phy->spi, REG_GAIN_TABLE_CONFIG, START_GAIN_TABLE_CLOCK |
		RECEIVER_SELECT(dest)); /* Start Gain Table Clock */

	for (i = 0; i < index_max; i++) {
		ad9361_spi_write(phy->spi, REG_GAIN_TABLE_ADDRESS, i); /* Gain Table Index */
		ad9361_spi_write(phy->spi, REG_GAIN_TABLE_WRITE_DATA1, tab[i][0]|lna); /* Ext LNA, Int LNA, & Mixer Gain Word */
		ad9361_spi_write(phy->spi, REG_GAIN_TABLE_WRITE_DATA2, tab[i][1]); /* TIA & LPF Word */
		ad9361_spi_write(phy->spi, REG_GAIN_TABLE_WRITE_DATA3, tab[i][2]); /* DC Cal bit & Dig Gain Word */
		ad9361_spi_write(phy->spi, REG_GAIN_TABLE_CONFIG,
			START_GAIN_TABLE_CLOCK |
			WRITE_GAIN_TABLE |
			RECEIVER_SELECT(dest)); /* Gain Table Index */
		ad9361_spi_write(phy->spi, REG_GAIN_TABLE_READ_DATA1, 0); /* Dummy Write to delay 3 ADCCLK/16 cycles */
		ad9361_spi_write(phy->spi, REG_GAIN_TABLE_READ_DATA1, 0); /* Dummy Write to delay ~1u */
	}
	...
	ad9361_spi_write(phy->spi, REG_GAIN_TABLE_READ_DATA1, 0); /* Dummy Write to delay ~1u */
	ad9361_spi_write(phy->spi, REG_GAIN_TABLE_READ_DATA1, 0); /* Dummy Write to delay ~1u */
	ad9361_spi_write(phy->spi, REG_GAIN_TABLE_CONFIG, 0); /* Stop Gain Table Clock */
...
int32_t ad9361_set_tx_atten(struct ad9361_rf_phy *phy, uint32_t atten_mdb,bool tx1, bool tx2, bool immed)
{
	...
	//dev_dbg(&phy->spi->dev, "%s : attenuation %"PRIu32" mdB tx1=%d tx2=%d",	__func__, atten_mdb, tx1, tx2);
	...
	ad9361_spi_writef(phy->spi, REG_TX2_DIG_ATTEN,IMMEDIATELY_UPDATE_TPC_ATTEN, 0);

	if (tx1)
	{
		//ret = ad9361_spi_writem(phy->spi, REG_TX1_ATTEN_1, buf, 2);
		ret = ad9361_spi_write(phy->spi, REG_TX1_ATTEN_1, buf[0]);
	    ret = ad9361_spi_write(phy->spi, REG_TX1_ATTEN_0, buf[1]);
	}


	if (tx2)
	{
		ret = ad9361_spi_write(phy->spi, REG_TX2_ATTEN_1, buf[0]);
	    ret = ad9361_spi_write(phy->spi, REG_TX2_ATTEN_0, buf[1]);
	}

	if (immed)
		ad9361_spi_writef(phy->spi, REG_TX2_DIG_ATTEN,IMMEDIATELY_UPDATE_TPC_ATTEN, 1);
...
//	ret = ad9361_spi_readm(par, (tx_num == 1) ?REG_TX1_ATTEN_1 : REG_TX2_ATTEN_1, buf, 2);
	buf[0] = ad9361_spi_read(phy->spi, (tx_num == 1) ?REG_TX1_ATTEN_1 : REG_TX2_ATTEN_1);
	buf[1] = ad9361_spi_read(phy->spi, (tx_num == 1) ?REG_TX1_ATTEN_0 : REG_TX2_ATTEN_0);
...
//			__func__, tab[i].VCO_MHz, i);
		...
		ad9361_spi_write(phy->spi, REG_RX_VCO_OUTPUT + offs,
			VCO_OUTPUT_LEVEL(tab[i].VCO_Output_Level) |
			PORB_VCO_LOGIC);
		ad9361_spi_writef(phy->spi, REG_RX_ALC_VARACTOR + offs,
			VCO_VARACTOR(~0), tab[i].VCO_Varactor);
		//ad9361_spi_write(par, REG_RX_ALC_VARACTOR + offs,0xF0);
		ad9361_spi_write(phy->spi, REG_RX_VCO_BIAS_1 + offs,
			VCO_BIAS_REF(tab[i].VCO_Bias_Ref) |
			VCO_BIAS_TCF(tab[i].VCO_Bias_Tcf));

		ad9361_spi_write(phy->spi, REG_RX_FORCE_VCO_TUNE_1 + offs,
			VCO_CAL_OFFSET(tab[i].VCO_Cal_Offset));
		ad9361_spi_write(phy->spi, REG_RX_VCO_VARACTOR_CTRL_1 + offs,
			VCO_VARACTOR_REFERENCE(
			tab[i].VCO_Varactor_Reference));

		ad9361_spi_write(phy->spi, REG_RX_VCO_CAL_REF + offs, VCO_CAL_REF_TCF(0));

		ad9361_spi_write(phy->spi, REG_RX_VCO_VARACTOR_CTRL_0 + offs,
			VCO_VARACTOR_OFFSET(0) |
			VCO_VARACTOR_REFERENCE_TCF(7));

		ad9361_spi_writef(phy->spi, REG_RX_CP_CURRENT + offs, CHARGE_PUMP_CURRENT(~0),
			tab[i].Charge_Pump_Current);
		ad9361_spi_write(phy->spi, REG_RX_LOOP_FILTER_1 + offs,
			LOOP_FILTER_C2(tab[i].LF_C2) |
			LOOP_FILTER_C1(tab[i].LF_C1));
		ad9361_spi_write(phy->spi, REG_RX_LOOP_FILTER_2 + offs,
			LOOP_FILTER_R1(tab[i].LF_R1) |
			LOOP_FILTER_C3(tab[i].LF_C3));
		ad9361_spi_write(phy->spi, REG_RX_LOOP_FILTER_3 + offs,
...
struct rf_rx_gain *rx_gain)
{
	...
	rx_gain->fgt_lmt_index = ad9361_spi_readf(phy->spi, idx_reg,
		FULL_TABLE_GAIN_INDEX(~0));
	tbl_addr = ad9361_spi_read(phy->spi, REG_GAIN_TABLE_ADDRESS);

	ad9361_spi_write(phy->spi, REG_GAIN_TABLE_ADDRESS, rx_gain->fgt_lmt_index);

	val = ad9361_spi_read(phy->spi, REG_GAIN_TABLE_READ_DATA1);
	rx_gain->lna_index = TO_LNA_GAIN(val);
	rx_gain->mixer_index = TO_MIXER_GM_GAIN(val);

	rx_gain->tia_index = ad9361_spi_readf(phy->spi, REG_GAIN_TABLE_READ_DATA2, TIA_GAIN);
	...
	ad9361_spi_write(phy->spi, REG_GAIN_TABLE_ADDRESS, tbl_addr);

	/* Read LPF Index */
	rx_gain->lpf_gain = ad9361_spi_readf(phy->spi, idx_reg + 1, LPF_GAIN_RX(~0));

	/* Read Digital Gain */
	rx_gain->digital_gain = ad9361_spi_readf(phy->spi, idx_reg + 2,
		DIGITAL_GAIN_RX(~0));
...
struct rf_rx_gain *rx_gain)
{
	...
	rx_gain->fgt_lmt_index = val = ad9361_spi_readf(phy->spi, idx_reg,FULL_TABLE_GAIN_INDEX(~0));//76
	gain_info = &phy->rx_gain[tbl];
	if (val > gain_info->idx_step_offset) {
		val = val - gain_info->idx_step_offset;                               //76;
	...
	uint32_t rx_id, struct rf_rx_gain *rx_gain)
{
	...
	val = ad9361_spi_readf(phy->spi, REG_RX_ENABLE_FILTER_CTRL, rx_enable_mask);//1
	...
	val = ad9361_spi_read(phy->spi, REG_AGC_CONFIG_1);//234

	val = (val >> gain_ctl_shift) & RX_GAIN_CTL_MASK;//2
	...
	if (val == RX_GAIN_CTL_AGC_FAST_ATK) {
		...
		*/
		val = ad9361_spi_read(phy->spi, REG_FAST_ATTACK_STATE);
...
void ad9361_ensm_force_state(struct ad9361_rf_phy *phy, uint8_t ensm_state)
{
	...
	dev_ensm_state = ad9361_spi_readf(phy->spi, REG_STATE, ENSM_STATE(~0));
...
//		ensm_state);
	...
	val = ad9361_spi_read(phy->spi, REG_ENSM_CONFIG_1);
	...
	*/
	...
	ad9361_spi_write(phy->spi, REG_ENSM_CONFIG_1, TO_ALERT | FORCE_ALERT_STATE);

	rc = ad9361_spi_write(phy->spi, REG_ENSM_CONFIG_1, val);
	if (rc)
...
struct rf_rx_gain *rx_gain)
{
	...
	ad9361_spi_writef(phy->spi, idx_reg, RX_FULL_TBL_IDX_MASK, rx_gain->fgt_lmt_index);
	ad9361_spi_writef(phy->spi, idx_reg + 1, RX_LPF_IDX_MASK, rx_gain->lpf_gain);

	if (phy->pdata->gain_ctrl.dig_gain_en) {
		ad9361_spi_writef(phy->spi, idx_reg + 2, RX_DIGITAL_IDX_MASK, rx_gain->digital_gain);
...
//		"Ignoring lmt/lpf/digital gains in Single Table mode");
	...
	ad9361_spi_writef(phy->spi, idx_reg, RX_FULL_TBL_IDX_MASK, val);
	...
	uint32_t rx_id, struct rf_rx_gain *rx_gain)
{
	...
	val = ad9361_spi_read(phy->spi, REG_AGC_CONFIG_1);
 ...
 */
/*
static int32_t ad9361_en_dis_tx(char par,struct ad9361_rf_phy *phy, uint32_t tx_if, uint32_t enable)
{
	if (tx_if == 2 && !phy->pdata->rx2tx2 && enable)
		return -EINVAL;

	return ad9361_spi_writef(par, REG_TX_ENABLE_FILTER_CTRL,
		TX_CHANNEL_ENABLE(tx_if), enable);
}
*/
/**
 ...
 */
/*
static int32_t ad9361_en_dis_rx(char par,struct ad9361_rf_phy *phy, uint32_t rx_if, uint32_t enable)
{
	if (rx_if == 2 && !phy->pdata->rx2tx2 && enable)
		return -EINVAL;

	return ad9361_spi_writef(par, REG_RX_ENABLE_FILTER_CTRL,
		RX_CHANNEL_ENABLE(rx_if), enable);
}
*/
/**
...
static int32_t ad9361_gc_update(struct ad9361_rf_phy *phy)
{
	...
	ret = ad9361_spi_writef(phy->spi, REG_AGC_ATTACK_DELAY,
	...
	ret |= ad9361_spi_writef(phy->spi, REG_FAST_CONFIG_2_SETTLING_DELAY,
	...
	ad9361_spi_writef(phy->spi, REG_DEC_POWER_MEASURE_DURATION_0,
			  DEC_POWER_MEASUREMENT_DURATION(~0),
			  ilog2(dec_pow_meas_dur / 16));


	ret |= ad9361_spi_writef(phy->spi, REG_DIGITAL_SAT_COUNTER,
			  DOUBLE_GAIN_COUNTER,  reg > 65535);

	if (reg > 65535)
		reg /= 2;

	ret |= ad9361_spi_write(phy->spi, REG_GAIN_UPDATE_COUNTER1, reg & 0xFF);
	ret |= ad9361_spi_write(phy->spi, REG_GAIN_UPDATE_COUNTER2, reg >> 8);
	...
	ret |= ad9361_spi_writef(phy->spi, REG_FAST_ENERGY_DETECT_COUNT,
		...
		struct rf_gain_ctrl *gain_ctrl)
{
	...
	rc = ad9361_spi_readm(phy->spi, REG_AGC_CONFIG_1, &val, 1);
	//rc = ad9361_spi_read(par, REG_AGC_CONFIG_1, &val, 1);
	if (rc) {
	...
	rc = ad9361_spi_write(phy->spi, REG_AGC_CONFIG_1, val);
	if (rc) {
...
int32_t ad9361_read_rssi(struct ad9361_rf_phy *phy, struct rf_rssi *rssi)
{
	...
	rc = ad9361_spi_readm(phy->spi, REG_PREAMBLE_LSB,reg_val_buf, ARRAY_SIZE(reg_val_buf));
	if (rssi->ant == 1) {
...
//		__func__, ref_clk_hz, tx);
	...
	/* REVIST:
	 * ad9361_spi_write(par, REG_RX_CP_LEVEL_DETECT + offs, 0x17);
	 */
	...
	/*ret =*/ ad9361_spi_write(phy->spi, REG_RX_CP_CONFIG + offs, 0x00);
	SysCtlDelay(20000);
	...
	SysCtlDelay(20000);//WAIT	1);// waits 1 ms);

	/*ret =*/ad9361_spi_write(phy->spi, REG_RX_CP_CONFIG + offs, CP_CAL_ENABLE);
	SysCtlDelay(20000);//WAIT	1);// waits 1 ms);
 ...
 * @return 0 in case of success, negative error code otherwise.
 */
static int32_t ad9361_rf_dc_offset_calib(struct ad9361_rf_phy *phy,uint64_t rx_freq)
{
	//struct spi_device *spi = phy->spi;
...
//		__func__, rx_freq);
	...
	ad9361_spi_write(phy->spi, REG_WAIT_COUNT, 0x20);

	if (rx_freq <= 4000000000ULL) {
		ad9361_spi_write(phy->spi, REG_RF_DC_OFFSET_COUNT,
			phy->pdata->rf_dc_offset_count_low);
		ad9361_spi_write(phy->spi, REG_RF_DC_OFFSET_CONFIG_1,
			RF_DC_CALIBRATION_COUNT(4) | DAC_FS(2));
		ad9361_spi_write(phy->spi, REG_RF_DC_OFFSET_ATTEN,
			RF_DC_OFFSET_ATTEN(
...
//			clktf * (txnco_word + 1) / 32, bw_tx / 4, txnco_word);
	...
	if (phy->pdata->rx1rx2_phase_inversion_en ||
		...
		ad9361_spi_writef(phy->spi, REG_PARALLEL_PORT_CONF_2, INVERT_RX2, 0);

		reg_inv_bits = ad9361_spi_read(phy->spi, REG_INVERT_BITS);
	...
	ad9361_spi_writef(phy->spi, REG_KEXP_2, TX_NCO_FREQ(~0), txnco_word);
	ad9361_spi_write(phy->spi, REG_QUAD_CAL_COUNT, 0xFF);
	ad9361_spi_write(phy->spi, REG_KEXP_1, KEXP_TX(1) | KEXP_TX_COMP(3) |
		KEXP_DC_I(3) | KEXP_DC_Q(3));
	ad9361_spi_write(phy->spi, REG_MAG_FTEST_THRESH, 0x01);
	ad9361_spi_write(phy->spi, REG_MAG_FTEST_THRESH_2, 0x01);
	...
	if ((tab[i][1] & lpf_tia_mask) == 0x20) {
		ad9361_spi_write(phy->spi, REG_TX_QUAD_FULL_LMT_GAIN, i);
		break;
	}
	...
	ad9361_spi_write(phy->spi, REG_QUAD_SETTLE_COUNT, 0xF0);
	ad9361_spi_write(phy->spi, REG_TX_QUAD_LPF_GAIN, 0x00);
...
//		!!(val & TX1_LO_CONV), !!(val & TX1_SSB_CONV), __rx_phase);
	...
	if (phy->pdata->rx1rx2_phase_inversion_en ||
		(phy->pdata->port_ctrl.pp_conf[1] & INVERT_RX2)) {
		ad9361_spi_writef(phy->spi, REG_PARALLEL_PORT_CONF_2, INVERT_RX2, 1);
		ad9361_spi_write(phy->spi, REG_INVERT_BITS, reg_inv_bits);
	}
...
//		__func__, bbdc_track, rfdc_track, rxquad_track);
	...
	ad9361_spi_writef(phy->spi, REG_RX_QUAD_GAIN2,
...
//		(phy->pdata->rx2tx2 ? ENABLE_TRACKING_MODE_CH2 : 0);
	...
	ad9361_spi_write(phy->spi, REG_CALIBRATION_CONFIG_1,
			       ...
			       struct tx_monitor_control *ctrl)
{
	...
	ad9361_spi_write(phy->spi, REG_TPM_MODE_ENABLE,
			 (ctrl->one_shot_mode_en ? ONE_SHOT_MODE : 0) |
			 TX_MON_DURATION(ilog2(ctrl->tx_mon_duration / 16)));

	ad9361_spi_write(phy->spi, REG_TX_MON_DELAY, ctrl->tx_mon_delay);

	ad9361_spi_write(phy->spi, REG_TX_MON_1_CONFIG,
			 TX_MON_1_LO_CM(ctrl->tx1_mon_lo_cm) |
			 TX_MON_1_GAIN(ctrl->tx1_mon_front_end_gain));
	ad9361_spi_write(phy->spi, REG_TX_MON_2_CONFIG,
			 TX_MON_2_LO_CM(ctrl->tx2_mon_lo_cm) |
			 TX_MON_2_GAIN(ctrl->tx2_mon_front_end_gain));

	ad9361_spi_write(phy->spi, REG_TX_ATTEN_THRESH,
			 ctrl->low_high_gain_threshold_mdB / 250);

	ad9361_spi_write(phy->spi, REG_TX_MON_HIGH_GAIN,
			 TX_MON_HIGH_GAIN(ctrl->high_gain_dB));

	ad9361_spi_write(phy->spi, REG_TX_MON_LOW_GAIN,
...
//	//struct spi_device *spi = phy->spi;
	...
	if (restore_c3) {
		return ad9361_spi_write(phy->spi, REG_PARALLEL_PORT_CONF_3,
	...
	ad9361_spi_write(phy->spi, REG_PARALLEL_PORT_CONF_1, pd->port_ctrl.pp_conf[0]);
	ad9361_spi_write(phy->spi, REG_PARALLEL_PORT_CONF_2, pd->port_ctrl.pp_conf[1]);
	ad9361_spi_write(phy->spi, REG_PARALLEL_PORT_CONF_3, pd->port_ctrl.pp_conf[2]);
	ad9361_spi_write(phy->spi, REG_RX_CLOCK_DATA_DELAY, pd->port_ctrl.rx_clk_data_delay);
	ad9361_spi_write(phy->spi, REG_TX_CLOCK_DATA_DELAY, pd->port_ctrl.tx_clk_data_delay);

	ad9361_spi_write(phy->spi, REG_LVDS_BIAS_CTRL, pd->port_ctrl.lvds_bias_ctrl);
	ad9361_spi_write(phy->spi, REG_DIGITAL_IO_CTRL, pd->port_ctrl.digital_io_ctrl);
	ad9361_spi_write(phy->spi, REG_LVDS_INVERT_CTRL1, pd->port_ctrl.lvds_invert[0]);
	ad9361_spi_write(phy->spi, REG_LVDS_INVERT_CTRL2, pd->port_ctrl.lvds_invert[1]);

	if (pd->rx1rx2_phase_inversion_en ||
		(pd->port_ctrl.pp_conf[1] & INVERT_RX2)) {

		ad9361_spi_writef(phy->spi, REG_PARALLEL_PORT_CONF_2, INVERT_RX2, 1);
		ad9361_spi_writef(phy->spi, REG_INVERT_BITS,
...
static int32_t ad9361_gc_setup(struct ad9361_rf_phy *phy, struct gain_control *ctrl)
{
	...
	ad9361_spi_write(phy->spi, REG_AGC_CONFIG_1, reg); // Gain Control Mode Select

	/* AGC_USE_FULL_GAIN_TABLE handled in ad9361_load_gt()*/
	ad9361_spi_writef(phy->spi, REG_AGC_CONFIG_2, MAN_GAIN_CTRL_RX1,ctrl->mgc_rx1_ctrl_inp_en);
	ad9361_spi_writef(phy->spi, REG_AGC_CONFIG_2, MAN_GAIN_CTRL_RX2,	ctrl->mgc_rx2_ctrl_inp_en);
	ad9361_spi_writef(phy->spi, REG_AGC_CONFIG_2, DIG_GAIN_EN,ctrl->dig_gain_en);
	...
	ad9361_spi_write(phy->spi, REG_AGC_CONFIG_3, reg); // Incr Step Size, ADC Overrange Size
	...
	ad9361_spi_write(phy->spi, REG_MAX_LMT_FULL_GAIN, reg); // Max Full/LMT Gain Table Index
	ad9361_spi_write(phy->spi, REG_RX1_MANUAL_LMT_FULL_GAIN, reg); // Rx1 Full/LMT Gain Index
	ad9361_spi_write(phy->spi, REG_RX2_MANUAL_LMT_FULL_GAIN, reg); // Rx2 Full/LMT Gain Index
	...
	ad9361_spi_write(phy->spi, REG_PEAK_WAIT_TIME, reg); // Decr Step Size, Peak Overload Time

	if (ctrl->dig_gain_en)
		ad9361_spi_write(phy->spi, REG_DIGITAL_GAIN,MAXIMUM_DIGITAL_GAIN(ctrl->max_dig_gain) |DIG_GAIN_STP_SIZE(ctrl->dig_gain_step_size));

	if (ctrl->adc_large_overload_thresh >= ctrl->adc_small_overload_thresh) {
		ad9361_spi_write(phy->spi, REG_ADC_SMALL_OVERLOAD_THRESH,
			ctrl->adc_small_overload_thresh); // ADC Small Overload Threshold
		ad9361_spi_write(phy->spi, REG_ADC_LARGE_OVERLOAD_THRESH,
	...
	ad9361_spi_write(phy->spi, REG_LARGE_LMT_OVERLOAD_THRESH, reg);
	reg = (ctrl->lmt_overload_low_thresh / 16) - 1;
	reg = clamp(reg, 0U, 63U);
	ad9361_spi_writef(phy->spi, REG_SMALL_LMT_OVERLOAD_THRESH,
		SMALL_LMT_OVERLOAD_THRESH(~0), reg);

	if (has_split_gt && phy->pdata->split_gt) {
		/* REVIST */
		ad9361_spi_write(phy->spi, REG_RX1_MANUAL_LPF_GAIN, 0x58); // Rx1 LPF Gain Index
		ad9361_spi_write(phy->spi, REG_RX2_MANUAL_LPF_GAIN, 0x18); // Rx2 LPF Gain Index
		ad9361_spi_write(phy->spi, REG_FAST_INITIAL_LMT_GAIN_LIMIT, 0x27); // Initial LMT Gain Limit
	}

	ad9361_spi_write(phy->spi, REG_RX1_MANUAL_DIGITALFORCED_GAIN, 0x00); // Rx1 Digital Gain Index
	ad9361_spi_write(phy->spi, REG_RX2_MANUAL_DIGITALFORCED_GAIN, 0x00); // Rx2 Digital Gain Index

	reg = clamp_t(uint8_t, ctrl->low_power_thresh, 0U, 64U) * 2;
	ad9361_spi_write(phy->spi, REG_FAST_LOW_POWER_THRESH, reg); // Low Power Threshold
	ad9361_spi_write(phy->spi, REG_TX_SYMBOL_ATTEN_CONFIG, 0x00); // Tx Symbol Gain Control
	...
	ad9361_spi_writef(phy->spi, REG_AGC_LOCK_LEVEL,
	...
	ad9361_spi_write(phy->spi, REG_GAIN_STP_2, reg);
	...
	ad9361_spi_write(phy->spi, REG_GAIN_STP1, reg);
	...
	ad9361_spi_write(phy->spi, REG_ADC_OVERLOAD_COUNTERS, reg);

	ad9361_spi_writef(phy->spi, REG_GAIN_STP_CONFIG_2, LARGE_LPF_GAIN_STEP(~0),
	...
	ad9361_spi_write(phy->spi, REG_LMT_OVERLOAD_COUNTERS, reg);

	ad9361_spi_writef(phy->spi, REG_GAIN_STP_CONFIG1,
	...
	ad9361_spi_write(phy->spi, REG_DIGITAL_SAT_COUNTER, reg);
	...
	*/
	...
	ad9361_spi_writef(phy->spi, REG_FAST_STRONG_SIGNAL_FREEZE,
	...
	reg = clamp_t(uint32_t, reg, 0U, 63U);
	ad9361_spi_writef(phy->spi, REG_FAST_GAIN_LOCK_EXIT_COUNT,
	...
	reg = !ctrl->f_agc_rst_gla_large_adc_overload_en;
	ad9361_spi_writef(phy->spi, REG_FAST_LOW_POWER_THRESH,
	...
	reg = clamp_t(uint32_t, reg, 0U, 15U);
	ad9361_spi_writef(phy->spi, REG_RX1_MANUAL_LPF_GAIN,
		POWER_MEAS_IN_STATE_5(~0), reg);
	ad9361_spi_writef(phy->spi, REG_RX1_MANUAL_LMT_FULL_GAIN,
...
static int32_t ad9361_auxdac_setup(struct ad9361_rf_phy *phy,struct auxdac_control *ctrl)
{
	...
	uint8_t tmp;
		...
		ad9361_spi_writef(phy->spi, REG_AUXDAC_ENABLE_CTRL,
			AUXDAC_AUTO_TX_BAR(~0) |
			AUXDAC_AUTO_RX_BAR(~0) |
			AUXDAC_INIT_BAR(~0),
			tmp); /* Auto Control */

		ad9361_spi_writef(phy->spi, REG_EXTERNAL_LNA_CTRL,
			AUXDAC_MANUAL_SELECT, ctrl->auxdac_manual_mode_en);
		ad9361_spi_write(phy->spi, REG_AUXDAC1_RX_DELAY, ctrl->dac1_rx_delay_us);
		ad9361_spi_write(phy->spi, REG_AUXDAC1_TX_DELAY, ctrl->dac1_tx_delay_us);
		ad9361_spi_write(phy->spi, REG_AUXDAC2_RX_DELAY, ctrl->dac2_rx_delay_us);
		ad9361_spi_write(phy->spi, REG_AUXDAC2_TX_DELAY, ctrl->dac2_tx_delay_us);
...
static int32_t ad9361_auxadc_setup(struct ad9361_rf_phy *phy,struct auxadc_control *ctrl,	uint32_t bbpll_freq)
{
	...
	ad9361_spi_write(phy->spi, REG_TEMP_OFFSET, ctrl->offset);
	ad9361_spi_write(phy->spi, REG_START_TEMP_READING, 0x00);
	ad9361_spi_write(phy->spi, REG_TEMP_SENSE2,
		MEASUREMENT_TIME_INTERVAL(val) |
		(ctrl->periodic_temp_measuremnt ?
	TEMP_SENSE_PERIODIC_ENABLE : 0));
	ad9361_spi_write(phy->spi, REG_TEMP_SENSOR_CONFIG,
		TEMP_SENSOR_DECIMATION(
		ilog2(ctrl->temp_sensor_decimation) - 8));
	ad9361_spi_write(phy->spi, REG_AUXADC_CLOCK_DIVIDER,
...
struct ctrl_outs_control *ctrl)
{
	...
	ad9361_spi_write(phy->spi, REG_CTRL_OUTPUT_POINTER, ctrl->index); // Ctrl Out index
	return ad9361_spi_write(phy->spi, REG_CTRL_OUTPUT_ENABLE, ctrl->en_mask); // Ctrl Out [7:0] output enable
	//ad9361_spi_write(par, REG_CTRL_OUTPUT_POINTER, 0x07); // Ctrl Out index
	//return ad9361_spi_write(par, REG_CTRL_OUTPUT_ENABLE, 0xFF); // Ctrl Out [7:0] output enable
}
...
static int32_t ad9361_gpo_setup(struct ad9361_rf_phy *phy, struct gpo_control *ctrl)
{
	...
	ad9361_spi_write(phy->spi, REG_AUTO_GPO,
	...
	ad9361_spi_write(phy->spi, REG_GPO_FORCE_AND_INIT,
	...
	ad9361_spi_write(phy->spi, REG_GPO0_RX_DELAY, ctrl->gpo0_rx_delay_us);
	ad9361_spi_write(phy->spi, REG_GPO0_TX_DELAY, ctrl->gpo0_tx_delay_us);
	ad9361_spi_write(phy->spi, REG_GPO1_RX_DELAY, ctrl->gpo1_rx_delay_us);
	ad9361_spi_write(phy->spi, REG_GPO1_TX_DELAY, ctrl->gpo1_tx_delay_us);
	ad9361_spi_write(phy->spi, REG_GPO2_RX_DELAY, ctrl->gpo2_rx_delay_us);
	ad9361_spi_write(phy->spi, REG_GPO2_TX_DELAY, ctrl->gpo2_tx_delay_us);
	ad9361_spi_write(phy->spi, REG_GPO3_RX_DELAY, ctrl->gpo3_rx_delay_us);
	ad9361_spi_write(phy->spi, REG_GPO3_TX_DELAY, ctrl->gpo3_tx_delay_us);
	...
	bool is_update)
{
	...
	ad9361_spi_write(phy->spi, REG_MEASURE_DURATION_23,
		(dur_buf[3] << 4) | dur_buf[2]); // RSSI Measurement Duration 2, 3
	ad9361_spi_write(phy->spi, REG_RSSI_WEIGHT_0, weight[0]); // RSSI Weighted Multiplier 0
	ad9361_spi_write(phy->spi, REG_RSSI_WEIGHT_1, weight[1]); // RSSI Weighted Multiplier 1
	ad9361_spi_write(phy->spi, REG_RSSI_WEIGHT_2, weight[2]); // RSSI Weighted Multiplier 2
	ad9361_spi_write(phy->spi, REG_RSSI_WEIGHT_3, weight[3]); // RSSI Weighted Multiplier 3
	ad9361_spi_write(phy->spi, REG_RSSI_DELAY, rssi_delay); // RSSI Delay
	ad9361_spi_write(phy->spi, REG_RSSI_WAIT_TIME, rssi_wait); // RSSI Wait

	temp = RSSI_MODE_SELECT(ctrl->restart_mode);
	if (ctrl->restart_mode == SPI_WRITE_TO_REGISTER)
		temp |= START_RSSI_MEAS;

	ret = ad9361_spi_write(phy->spi, REG_RSSI_CONFIG, temp); // RSSI Mode Select
...
//		ensm_state);
	...
	if (phy->curr_ensm_state == ENSM_STATE_SLEEP) {
		ad9361_spi_write(phy->spi, REG_CLOCK_ENABLE,
	...
	case ENSM_STATE_SLEEP:
		...
		ad9361_spi_write(phy->spi, REG_ENSM_CONFIG_1, 0); /* Clear To Alert */
		ad9361_spi_write(phy->spi, REG_ENSM_CONFIG_1,
			phy->pdata->fdd ? FORCE_TX_ON : FORCE_RX_ON);
		/* Delay Flush Time 384 ADC clock cycles */
		SysCtlDelay(384000000UL / clk_get_rate(phy, phy->ref_clk_scale[ADC_CLK]));
		ad9361_spi_write(phy->spi, REG_ENSM_CONFIG_1, 0); /* Move to Wait*/
		SysCtlDelay(200000); /* Wait for ENSM settle */
		ad9361_spi_write(phy->spi, REG_CLOCK_ENABLE,
	...
	if ((val & FORCE_RX_ON) &&
		...
		tmp = ad9361_spi_read(phy->spi, REG_SMALL_LMT_OVERLOAD_THRESH);
		ad9361_spi_write(phy->spi, REG_SMALL_LMT_OVERLOAD_THRESH,
...
/*
		if (!phy->pdata->dig_interface_tune_fir_disable &&
			...
			ret = ad9361_dig_tune(par,phy, 0, SKIP_STORE_RESULT);
*/
...
//		__func__, tx ? "TX" : "RX", profile);
	...
	val[0] = ad9361_spi_read(phy->spi, REG_RX_INTEGER_BYTE_0 + offs);
	val[1] = ad9361_spi_read(phy->spi, REG_RX_INTEGER_BYTE_1 + offs);
	val[2] = ad9361_spi_read(phy->spi, REG_RX_FRACT_BYTE_0 + offs);
	val[3] = ad9361_spi_read(phy->spi, REG_RX_FRACT_BYTE_1 + offs);
	val[4] = ad9361_spi_read(phy->spi, REG_RX_FRACT_BYTE_2 + offs);

	x = ad9361_spi_readf(phy->spi, REG_RX_VCO_BIAS_1 + offs, VCO_BIAS_REF(~0));
	y = ad9361_spi_readf(phy->spi, REG_RX_ALC_VARACTOR + offs, VCO_VARACTOR(~0));
	val[5] = (x << 4) | y;

	x = ad9361_spi_readf(phy->spi, REG_RX_VCO_BIAS_1 + offs, VCO_BIAS_TCF(~0));
	y = ad9361_spi_readf(phy->spi, REG_RX_CP_CURRENT + offs, CHARGE_PUMP_CURRENT(~0));
	/* Wide BW option: N = 1
	...
	*/
	...
	x = ad9361_spi_readf(phy->spi, REG_RX_LOOP_FILTER_3 + offs, LOOP_FILTER_R3(~0));
	val[8] = (x << 4) | x;

	x = ad9361_spi_readf(phy->spi, REG_RX_LOOP_FILTER_2 + offs, LOOP_FILTER_C3(~0));
	val[9] = (x << 4) | x;

	x = ad9361_spi_readf(phy->spi, REG_RX_LOOP_FILTER_1 + offs, LOOP_FILTER_C1(~0));
	y = ad9361_spi_readf(phy->spi, REG_RX_LOOP_FILTER_1 + offs, LOOP_FILTER_C2(~0));
	val[10] = (x << 4) | y;

	x = ad9361_spi_readf(phy->spi, REG_RX_LOOP_FILTER_2 + offs, LOOP_FILTER_R1(~0));
	val[11] = (x << 4) | x;

	x = ad9361_spi_readf(phy->spi, REG_RX_VCO_VARACTOR_CTRL_0 + offs,
		VCO_VARACTOR_REFERENCE_TCF(~0));
	y = ad9361_spi_readf(phy->spi, REG_RFPLL_DIVIDERS,
		tx ? TX_VCO_DIVIDER(~0) : RX_VCO_DIVIDER(~0));
	val[12] = (x << 4) | y;

	x = ad9361_spi_readf(phy->spi, REG_RX_FORCE_VCO_TUNE_1 + offs, VCO_CAL_OFFSET(~0));
	y = ad9361_spi_readf(phy->spi, REG_RX_VCO_VARACTOR_CTRL_1 + offs, VCO_VARACTOR_REFERENCE(~0));
	val[13] = (x << 4) | y;

	val[14] = ad9361_spi_read(phy->spi, REG_RX_FORCE_VCO_TUNE_0 + offs);

	x = ad9361_spi_readf(phy->spi, REG_RX_FORCE_ALC + offs, FORCE_ALC_WORD(~0));
	y = ad9361_spi_readf(phy->spi, REG_RX_FORCE_VCO_TUNE_1 + offs, FORCE_VCO_TUNE);
	val[15] = (x << 1) | y;
...
int32_t ad9361_setup(struct ad9361_rf_phy *phy)
{
	...
	ad9361_spi_write(phy->spi, REG_CTRL, CTRL_ENABLE);																//83DF01
	ad9361_spi_write(phy->spi, REG_BANDGAP_CONFIG0, MASTER_BIAS_TRIM(0x0E)); /* Enable Master Bias */                //82A60E
	ad9361_spi_write(phy->spi, REG_BANDGAP_CONFIG1, BANDGAP_TEMP_TRIM(0x0E)); /* Set Bandgap Trim */					//82A80E

	//ad9361_spi_write(par, REG_DCXO_COARSE_TUNE, 0x08); /* Set Bandgap Trim */
	//ad9361_spi_write(par, REG_DCXO_FINE_TUNE_LOW, 0x00); /* Set Bandgap Trim */
	//ad9361_spi_write(par, REG_DCXO_FINE_TUNE_HIGH, 0xB9); /* Set Bandgap Trim */
	ad9361_set_dcxo_tune(phy, pd->dcxo_coarse, pd->dcxo_fine);													//829208;829400;8293B9

	refin_Hz = phy->clk_refin->rate;//40MHZ
	...
	ad9361_spi_writef(phy->spi, REG_REF_DIVIDE_CONFIG_1, RX_REF_RESET_BAR, 1);                                    //82AB06
		ad9361_spi_writef(phy->spi, REG_REF_DIVIDE_CONFIG_2, TX_REF_RESET_BAR, 1);                                   //82AC10
		ad9361_spi_writef(phy->spi, REG_REF_DIVIDE_CONFIG_2,                                                         //82AC13
			TX_REF_DOUBLER_FB_DELAY(~0), 3); /* FB DELAY */                                                     //82AC73
		ad9361_spi_writef(phy->spi, REG_REF_DIVIDE_CONFIG_2,
			RX_REF_DOUBLER_FB_DELAY(~0), 3); /* FB DELAY */

	//ad9361_spi_write(par, REG_REF_DIVIDE_CONFIG_1, 0x06);                                    //82AB06
	//ad9361_spi_write(par, REG_REF_DIVIDE_CONFIG_2,0x10);                                   //82AC10
	//ad9361_spi_write(par, REG_REF_DIVIDE_CONFIG_2,0x13);                                                      //82AC13
	//ad9361_spi_write(par, REG_REF_DIVIDE_CONFIG_2,0x73); /* FB DELAY */                                       //82AC73

	ad9361_spi_write(phy->spi, REG_CLOCK_ENABLE,
		DIGITAL_POWER_UP | CLOCK_ENABLE_DFLT | BBPLL_ENABLE |
		(pd->use_extclk ? XO_BYPASS : 0)); /* Enable Clocks */                                            //800907
	...
	ret = ad9361_set_trx_clock_chain(phy, pd->rx_path_clks,                                           //NEXT REGISTER VALUES FRO HERE;
		pd->tx_path_clks);
	if (ret < 0)
		return ret;
	...
	if (ret < 0)
		return ret;
	//	 ad9361_spi_write(par,REG_RX_VCO_OUTPUT,0x4A);// Set VCO Output level[3:0]
	//		 	ad9361_spi_write(par,REG_RX_ALC_VARACTOR,0xC0);// Set Init ALC Value[3:0] and VCO Varactor[3:0]
	//		 	ad9361_spi_write(par,REG_RX_VCO_BIAS_1,0x0D);// Set VCO Bias Tcf[1:0] and VCO Bias Ref[2:0]
	//		 	ad9361_spi_write(par,REG_RX_FORCE_VCO_TUNE_1,0x68);// Set VCO Cal Offset[3:0]
	//		 	ad9361_spi_write(par,REG_RX_VCO_CAL_REF,0x00);// Set VCO Cal Ref Tcf[2:0]
	//		 	ad9361_spi_write(par,REG_RX_VCO_VARACTOR_CTRL_1,0x09);// Set VCO Varactor Reference[3:0]
	//		 	ad9361_spi_write(par,REG_RX_VCO_VARACTOR_CTRL_0,0x70);// Set VCO Varactor Ref Tcf[2:0] and VCO Varactor Offset[3:0]
	//		 	ad9361_spi_write(par,REG_RX_CP_CURRENT,0x91);// Set Synth Loop Filter charge pump current (Icp)
	//		 	ad9361_spi_write(par,REG_RX_LOOP_FILTER_1,0xD4);// Set Synth Loop Filter C2 and C1
	//		 	ad9361_spi_write(par,REG_RX_LOOP_FILTER_2,0xDF);// Set Synth Loop Filter  R1 and C3
	//		 	ad9361_spi_write(par,REG_RX_LOOP_FILTER_3,0x09);// Set Synth Loop Filter R3
	//
	//		 	// ************************************************************
	//		 	// Setup Synthesizer
	//		 	// ************************************************************
	//		 	ad9361_spi_write(par,REG_TX_VCO_OUTPUT,0x4A);// Set VCO Output level[3:0]
	//		 	ad9361_spi_write(par,REG_TX_ALCVARACT_OR,0xC0);// Set Init ALC Value[3:0] and VCO Varactor[3:0]
	//		 	ad9361_spi_write(par,REG_TX_VCO_BIAS_1,0x0D);// Set VCO Bias Tcf[1:0] and VCO Bias Ref[2:0]
	//		 	ad9361_spi_write(par,REG_TX_FORCE_VCO_TUNE_1,0x68);// Set VCO Cal Offset[3:0]
	//		 	ad9361_spi_write(par,REG_TX_VCO_CAL_REF,0x00);// Set VCO Cal Ref Tcf[2:0]
	//		 	ad9361_spi_write(par,REG_TX_VCO_VARACTOR_CTRL_1,0x09);// Set VCO Varactor Reference[3:0]
	//		 	ad9361_spi_write(par,REG_TX_VCO_VARACTOR_CTRL_0,0x70);// Set VCO Varactor Ref Tcf[2:0] and VCO Varactor Offset[3:0]
	//		 	ad9361_spi_write(par,REG_TX_CP_CURRENT,0x91);// Set Synth Loop Filter charge pump current (Icp)
	//		 	ad9361_spi_write(par,REG_TX_LOOP_FILTER_1,0xD4);// Set Synth Loop Filter C2 and C1
	//		 	ad9361_spi_write(par,REG_TX_LOOP_FILTER_2,0xDF);// Set Synth Loop Filter  R1 and C3
	//		 	ad9361_spi_write(par,REG_TX_LOOP_FILTER_3,0x09);// Set Synth Loop Filter R3
	//
	//
	//
	//		 	ad9361_spi_write(par,REG_RX_FRACT_BYTE_0,0x00	);//  Write Rx Synth Fractional Freq Word[7:0]
	//		 	ad9361_spi_write(par,REG_RX_FRACT_BYTE_1,0x00	);//  Write Rx Synth Fractional Freq Word[15:8]
	//		 	ad9361_spi_write(par,REG_RX_FRACT_BYTE_2,0x00	);//  Write Rx Synth Fractional Freq Word[22:16]
	//		 	ad9361_spi_write(par,REG_RX_INTEGER_BYTE_1,0x00	);//  Write Rx Synth Integer Freq Word[10:8]
	//		 	ad9361_spi_write(par,REG_RX_INTEGER_BYTE_0,0x78	);//  Write Rx Synth Integer Freq Word[7:0]
	//		 	ad9361_spi_write(par,REG_RFPLL_DIVIDERS,0x11	);//  Set LO divider setting
	//		 	ad9361_spi_write(par,REG_TX_FRACT_BYTE_0,0x00	);//  Write Tx Synth Fractional Freq Word[7:0]
	//		 	ad9361_spi_write(par,REG_TX_FRACT_BYTE_1,0x00	);//  Write Tx Synth Fractional Freq Word[15:8]
	//		 	ad9361_spi_write(par,REG_TX_FRACT_BYTE_2,0x00	);//  Write Tx Synth Fractional Freq Word[22:16]
	//		 	ad9361_spi_write(par,REG_TX_INTEGER_BYTE_1,0x00	);//  Write Tx Synth Integer Freq Word[10:8]
	//		 	ad9361_spi_write(par,REG_TX_INTEGER_BYTE_0,0x78	);//  Write Tx Synth Integer Freq Word[7:0] (starts VCO cal)
	//		 	ad9361_spi_write(par,REG_RFPLL_DIVIDERS,0x11	);//  Set LO divider setting
	//
	//		     ad9361_spi_read(par,REG_RX_CP_OVERRANGE_VCO_LOCK);//
	//		     ad9361_spi_read(par,REG_TX_CP_OVERRANGE_VCO_LOCK);//
	ret = clk_set_rate(phy, phy->ref_clk_scale[RX_RFPLL], ad9361_to_clk(pd->rx_synth_freq));// 0247 calibration timeout
			     	     	if (ret < 0) {
			     	     	...
			     	     	phy->last_tx_quad_cal_freq = pd->tx_synth_freq;//
			     	     	 ret = ad9361_spi_read(phy->spi,REG_TX_CP_OVERRANGE_VCO_LOCK);//
			     	     	ret = clk_set_rate(phy, phy->ref_clk_scale[TX_RFPLL], ad9361_to_clk(pd->tx_synth_freq));//  0287 calibration timeout
			     	     	if (ret < 0) {
	    ...
	    //	ad9361_clk_mux_set_parent(par,phy->ref_clk_scale[RX_RFPLL],
	    //		pd->use_ext_rx_lo);

	   // 	ad9361_clk_mux_set_parent(par,phy->ref_clk_scale[TX_RFPLL],
	   // 		pd->use_ext_tx_lo);
	...
	ret = ad9361_rx_bb_analog_filter_calib(phy,real_rx_bandwidth,bbpll_freq);
	if (ret < 0)
		return ret;

	ret = ad9361_tx_bb_analog_filter_calib(phy,real_tx_bandwidth,bbpll_freq);
	if (ret < 0)
		return ret;
	...
	phy->curr_ensm_state = ad9361_spi_readf(phy->spi, REG_STATE, ENSM_STATE(~0));
	ad9361_ensm_set_state(phy, pd->fdd ? ENSM_STATE_FDD : ENSM_STATE_RX,
		pd->ensm_pin_ctrl);

	phy->auto_cal_en = true;
	phy->cal_threshold_freq = 100000000ULL;
/*
		ad9361_spi_write(par, REG_BIST_CONFIG,0x53);
		ad9361_spi_write(par,REG_OBSERVE_CONFIG,0x41);
		ad9361_spi_write(par,REG_BIST_AND_DATA_PORT_TEST_CONFIG,0x00);
*/

	//SysCtlDelay(100000);
	//ret = ad9361_tx_quad_calib(1,phy, real_rx_bandwidth, real_tx_bandwidth, -1);
	//SysCtlDelay(1*16000000);
	//ret = ad9361_tx_quad_calib(2,phy, real_rx_bandwidth, real_tx_bandwidth, -1);
	//SysCtlDelay(100000);
	return 0;
...
int32_t ad9361_do_calib_run( struct ad9361_rf_phy *phy, uint32_t cal, int32_t arg)
{
	...
	switch (cal) {
	case TX_QUAD_CAL:
		ret = ad9361_tx_quad_calib(phy, phy->current_rx_bw_Hz / 2,phy->current_tx_bw_Hz / 2, arg);
		break;
	case RFDC_CAL:
		ret = ad9361_rf_dc_offset_calib(phy,ad9361_from_clk(clk_get_rate(phy, phy->ref_clk_scale[RX_RFPLL])));
		break;
...
//		__func__, ntaps, dest);
	...
	if (dest & FIR_IS_RX) {
		gain = ad9361_spi_read(phy->spi, REG_RX_FILTER_GAIN);
		offs = REG_RX_FILTER_COEF_ADDR - REG_TX_FILTER_COEF_ADDR;
		ad9361_spi_write(phy->spi, REG_RX_FILTER_GAIN, 0);
	}

	conf = ad9361_spi_read(phy->spi, REG_TX_FILTER_CONF + offs);
	...
	for (; cnt > 0; cnt--, sel++) {
		...
		ad9361_spi_write(phy->spi, REG_TX_FILTER_CONF + offs,
				 FIR_NUM_TAPS(ntaps / 16 - 1) |
				 FIR_SELECT(sel) | FIR_START_CLK);
		for (val = 0; val < ntaps; val++) {
			short tmp;
			ad9361_spi_write(phy->spi, REG_TX_FILTER_COEF_ADDR + offs, val);

			tmp = (ad9361_spi_read(phy->spi, REG_TX_FILTER_COEF_READ_DATA_1 + offs) & 0xFF) |
			(ad9361_spi_read(phy->spi, REG_TX_FILTER_COEF_READ_DATA_2 + offs) << 8);
	...
	if (dest & FIR_IS_RX) {
		ad9361_spi_write(phy->spi, REG_RX_FILTER_GAIN, gain);
	}

	ad9361_spi_write(phy->spi, REG_TX_FILTER_CONF + offs, conf);
...
//		__func__, ntaps, gain_dB, dest);
	...
	if (dest & FIR_IS_RX) {
		val = 3 - (gain_dB + 12) / 6;
		ad9361_spi_write(phy->spi, REG_RX_FILTER_GAIN, val & 0x3);
		offs = REG_RX_FILTER_COEF_ADDR - REG_TX_FILTER_COEF_ADDR;
	...
	ad9361_spi_write(phy->spi, REG_TX_FILTER_CONF + offs, fir_conf);

	for (val = 0; val < ntaps; val++) {
		ad9361_spi_write(phy->spi, REG_TX_FILTER_COEF_ADDR + offs, val);
		ad9361_spi_write(phy->spi, REG_TX_FILTER_COEF_WRITE_DATA_1 + offs,
			coef[val] & 0xFF);
		ad9361_spi_write(phy->spi, REG_TX_FILTER_COEF_WRITE_DATA_2 + offs,
			coef[val] >> 8);
		ad9361_spi_write(phy->spi, REG_TX_FILTER_CONF + offs,
			fir_conf | FIR_WRITE);
		ad9361_spi_write(phy->spi, REG_TX_FILTER_COEF_READ_DATA_2 + offs, 0);
		ad9361_spi_write(phy->spi, REG_TX_FILTER_COEF_READ_DATA_2 + offs, 0);
	}
...
/*
	if (phy->rx_fir_dec == 1 || phy->bypass_rx_fir) {
		ad9361_spi_writef(par, REG_RX_ENABLE_FILTER_CTRL,
			RX_FIR_ENABLE_DECIMATION(~0), !phy->bypass_rx_fir);
	}

	if (phy->tx_fir_int == 1 || phy->bypass_tx_fir) {
		ad9361_spi_writef(par, REG_TX_ENABLE_FILTER_CTRL,
			TX_FIR_ENABLE_INTERPOLATION(~0), !phy->bypass_tx_fir);
	}
*/
	/*
...
static int32_t ad9361_get_clk_scaler(struct refclk_scale *clk_priv)
{
	...
	case BB_REFCLK:
		tmp = ad9361_spi_read(clk_priv->spi, REG_CLOCK_CTRL);
		tmp &= 0x3;
		break;
	case RX_REFCLK:
		tmp = ad9361_spi_readf(clk_priv->spi, REG_REF_DIVIDE_CONFIG_1,
			RX_REF_DIVIDER_MSB);
		tmp1 = ad9361_spi_readf(clk_priv->spi, REG_REF_DIVIDE_CONFIG_2,
			RX_REF_DIVIDER_LSB);
		tmp = (tmp << 1) | tmp1;
		break;
	case TX_REFCLK:
		tmp = ad9361_spi_readf(clk_priv->spi, REG_REF_DIVIDE_CONFIG_2,
			TX_REF_DIVIDER(~0));
		break;
	case ADC_CLK:
		tmp = ad9361_spi_read(clk_priv->spi, REG_BBPLL);
		return ad9361_set_muldiv(clk_priv, 1, 1 << (tmp & 0x7));
	case R2_CLK:
		tmp = ad9361_spi_readf(clk_priv->spi, REG_RX_ENABLE_FILTER_CTRL,
			DEC3_ENABLE_DECIMATION(~0));
		return ad9361_set_muldiv(clk_priv, 1, tmp + 1);
	case R1_CLK:
		tmp = ad9361_spi_readf(clk_priv->spi, REG_RX_ENABLE_FILTER_CTRL, RHB2_EN);
		return ad9361_set_muldiv(clk_priv, 1, tmp + 1);
	case CLKRF_CLK:
		tmp = ad9361_spi_readf(clk_priv->spi, REG_RX_ENABLE_FILTER_CTRL, RHB1_EN);
		return ad9361_set_muldiv(clk_priv, 1, tmp + 1);
	case RX_SAMPL_CLK:
		tmp = ad9361_spi_readf(clk_priv->spi, REG_RX_ENABLE_FILTER_CTRL,
			RX_FIR_ENABLE_DECIMATION(~0));
	...
	case DAC_CLK:
		tmp = ad9361_spi_readf(clk_priv->spi, REG_BBPLL, BIT(3));
		return ad9361_set_muldiv(clk_priv, 1, tmp + 1);
	case T2_CLK:
		tmp = ad9361_spi_readf(clk_priv->spi, REG_TX_ENABLE_FILTER_CTRL,
			THB3_ENABLE_INTERP(~0));
		return ad9361_set_muldiv(clk_priv, 1, tmp + 1);
	case T1_CLK:
		tmp = ad9361_spi_readf(clk_priv->spi, REG_TX_ENABLE_FILTER_CTRL, THB2_EN);
		return ad9361_set_muldiv(clk_priv, 1, tmp + 1);
	case CLKTF_CLK:
		tmp = ad9361_spi_readf(clk_priv->spi, REG_TX_ENABLE_FILTER_CTRL, THB1_EN);
		return ad9361_set_muldiv(clk_priv, 1, tmp + 1);
	case TX_SAMPL_CLK:
		tmp = ad9361_spi_readf(clk_priv->spi, REG_TX_ENABLE_FILTER_CTRL,
			TX_FIR_ENABLE_INTERPOLATION(~0));
...
static int32_t ad9361_set_clk_scaler( struct refclk_scale *clk_priv, bool set)
{
	...
	case BB_REFCLK:
		...
		if (ret < 0)
			return ret;
		if (set)
			return ad9361_spi_writef(clk_priv->spi, REG_CLOCK_CTRL,
	...
	case RX_REFCLK:
		...
		if (set) {
			tmp = ret;
			ret = ad9361_spi_writef(clk_priv->spi, REG_REF_DIVIDE_CONFIG_1,
				RX_REF_DIVIDER_MSB, tmp >> 1);
			ret |= ad9361_spi_writef(clk_priv->spi, REG_REF_DIVIDE_CONFIG_2,
	...
	case TX_REFCLK:
		...
		if (ret < 0)
			return ret;
		if (set)
			return ad9361_spi_writef(clk_priv->spi, REG_REF_DIVIDE_CONFIG_2,
	...
	case ADC_CLK:
		...
		if (set)
			return ad9361_spi_writef(clk_priv->spi, REG_BBPLL, 0x7, tmp);
		break;
	case R2_CLK:
		if (clk_priv->mult != 1 || clk_priv->div > 3 || clk_priv->div < 1)
			return -EINVAL;
		if (set)
			return ad9361_spi_writef(clk_priv->spi, REG_RX_ENABLE_FILTER_CTRL,
			DEC3_ENABLE_DECIMATION(~0),
			clk_priv->div - 1);
		break;
	case R1_CLK:
	...
	case DAC_CLK:
		if (clk_priv->mult != 1 || clk_priv->div > 2 || clk_priv->div < 1)
			return -EINVAL;
		if (set)
			return ad9361_spi_writef(clk_priv->spi, REG_BBPLL,
	...
	case T2_CLK:
		if (clk_priv->mult != 1 || clk_priv->div > 3 || clk_priv->div < 1)
			return -EINVAL;
		if (set)
			return ad9361_spi_writef(clk_priv->spi, REG_TX_ENABLE_FILTER_CTRL,
			THB3_ENABLE_INTERP(~0),
			clk_priv->div - 1);
		break;
	case T1_CLK:
...
//	ad9361_spi_readm(clk_priv->spi, REG_INTEGER_BB_FREQ_WORD, &buf[0],
    ...
    buf[0]=ad9361_spi_read(clk_priv->spi, REG_INTEGER_BB_FREQ_WORD);
    buf[3]=ad9361_spi_read(clk_priv->spi, REG_FRACT_BB_FREQ_WORD_1);
    buf[2]=ad9361_spi_read(clk_priv->spi, REG_FRACT_BB_FREQ_WORD_2);
    buf[1]=ad9361_spi_read(clk_priv->spi, REG_FRACT_BB_FREQ_WORD_3);
...
//		__func__, rate, parent_rate);
	...
	*/
	...
	SysCtlDelay(20000);//WAIT	20);// waits 20 ms);
	ad9361_spi_write(clk_priv->spi, REG_CP_CURRENT, icp_val);                      //CP current setting 804604
	//ad9361_spi_writem(par, REG_LOOP_FILTER_3, lf_defaults,ARRAY_SIZE(lf_defaults));               //register 74 setting
	ad9361_spi_write(clk_priv->spi, REG_LOOP_FILTER_3,lf_defaults[0]);
	ad9361_spi_write(clk_priv->spi, REG_LOOP_FILTER_2,lf_defaults[1]);
	ad9361_spi_write(clk_priv->spi, REG_LOOP_FILTER_1,lf_defaults[2]);

	/* Allow calibration to occur and set cal count to 1024 for max accuracy */
	ad9361_spi_write(clk_priv->spi, REG_VCO_CTRL,
		FREQ_CAL_ENABLE | FREQ_CAL_COUNT_LENGTH(3));                    //804BE0
	/* Set calibration clock to REFCLK/4 for more accuracy */
	ad9361_spi_write(clk_priv->spi, REG_SDM_CTRL, 0x10);                          //804E10
	...
	fract = tmp;
////////////////////////////////////////BBPLL FREQUENCY SET///////////////////////////////////////////////////
	ad9361_spi_write(clk_priv->spi, REG_INTEGER_BB_FREQ_WORD, integer);                                        //804418
	ad9361_spi_write(clk_priv->spi, REG_FRACT_BB_FREQ_WORD_3, fract);                                          //804329
	ad9361_spi_write(clk_priv->spi, REG_FRACT_BB_FREQ_WORD_2, fract >> 8);                                     //80425C
	ad9361_spi_write(clk_priv->spi, REG_FRACT_BB_FREQ_WORD_1, fract >> 16);                                    //804112

	ad9361_spi_write(clk_priv->spi, REG_SDM_CTRL_1, INIT_BB_FO_CAL | BBPLL_RESET_BAR); /* Start BBPLL Calibration *///803F05
	ad9361_spi_write(clk_priv->spi, REG_SDM_CTRL_1, BBPLL_RESET_BAR); /* Clear BBPLL start calibration bit */   //803F01

	ad9361_spi_write(clk_priv->spi, REG_VCO_PROGRAM_1, 0x86); /* Increase BBPLL KV and phase margin */          //804C86
	ad9361_spi_write(clk_priv->spi, REG_VCO_PROGRAM_2, 0x01); /* Increase BBPLL KV and phase margin */          //804D01
	ad9361_spi_write(clk_priv->spi, REG_VCO_PROGRAM_2, 0x05); /* Increase BBPLL KV and phase margin */          //804D05

	return ad9361_check_cal_done(clk_priv->phy, REG_CH_1_OVERFLOW,BBPLL_LOCK, 1);                   //READ REGISTER 05E FOR CHECKING THE CALIBRATION IS DONE OR NOT
}
 ...
 */
static uint64_t ad9361_calc_rfpll_freq(uint64_t parent_rate, uint64_t integer, uint64_t fract, uint32_t vco_div)
{
	uint64_t rate;
	...
	uint32_t parent_rate)
{
	struct ad9361_rf_phy *phy = clk_priv->phy;
				uint64_t vco = 0;
				uint8_t buf[5];
				uint32_t reg0,reg1,reg2,reg3,reg4, div_mask, lock_reg, fract = 0, integer = 0;
...
//					__func__, rate, parent_rate);
				...
				ret = ad9361_calc_rfpll_divder(ad9361_from_clk(rate), parent_rate,&integer, &fract, &vco_div, &vco);
				if (ret < 0)
					return ret;

				switch (clk_priv->source) {
				case RX_RFPLL_INT:
			 			//ad9361_spi_write(clk_priv->spi,REG_RX_VCO_OUTPUT,0x4A);// Set VCO Output level[3:0]
					 	//ad9361_spi_write(clk_priv->spi,REG_RX_ALC_VARACTOR,0x81);// Set Init ALC Value[3:0] and VCO Varactor[3:0]
					 	//ad9361_spi_write(clk_priv->spi,REG_RX_VCO_BIAS_1,0x0E);// Set VCO Bias Tcf[1:0] and VCO Bias Ref[2:0]
					 	//ad9361_spi_write(clk_priv->spi,REG_RX_FORCE_VCO_TUNE_1,0x78);// Set VCO Cal Offset[3:0]
					 	//ad9361_spi_write(clk_priv->spi,REG_RX_VCO_CAL_REF,0x00);// Set VCO Cal Ref Tcf[2:0]
					 	//ad9361_spi_write(clk_priv->spi,REG_RX_VCO_VARACTOR_CTRL_1,0x0B);// Set VCO Varactor Reference[3:0]
					 	//ad9361_spi_write(clk_priv->spi,REG_RX_VCO_VARACTOR_CTRL_0,0x70);// Set VCO Varactor Ref Tcf[2:0] and VCO Varactor Offset[3:0]
					 	//ad9361_spi_write(clk_priv->spi,REG_RX_CP_CURRENT,0x8F);// Set Synth Loop Filter charge pump current (Icp)
					 	//ad9361_spi_write(clk_priv->spi,REG_RX_LOOP_FILTER_1,0xD4);// Set Synth Loop Filter C2 and C1
					 	//ad9361_spi_write(clk_priv->spi,REG_RX_LOOP_FILTER_2,0xDF);// Set Synth Loop Filter  R1 and C3
					 	//ad9361_spi_write(clk_priv->spi,REG_RX_LOOP_FILTER_3,0x09);// Set Synth Loop Filter R3

					 	//ad9361_spi_write(clk_priv->spi,REG_RFPLL_DIVIDERS,0x01);// Set Synth Loop Filter R3

					reg3 = REG_RX_INTEGER_BYTE_0;
					reg4 = REG_RX_INTEGER_BYTE_1;
					reg0 = REG_RX_FRACT_BYTE_0;
					reg1 = REG_RX_FRACT_BYTE_1;
					reg2 = REG_RX_FRACT_BYTE_2;
					lock_reg = REG_RX_CP_OVERRANGE_VCO_LOCK;
					div_mask = RX_VCO_DIVIDER(~0);
					break;
				case TX_RFPLL_INT:
				 		//ad9361_spi_write(clk_priv->spi,REG_TX_VCO_OUTPUT,0x4A);// Set VCO Output level[3:0]
					 	//ad9361_spi_write(clk_priv->spi,REG_TX_ALCVARACT_OR,0x81);// Set Init ALC Value[3:0] and VCO Varactor[3:0]
					 	//ad9361_spi_write(clk_priv->spi,REG_TX_VCO_BIAS_1,0x0E);// Set VCO Bias Tcf[1:0] and VCO Bias Ref[2:0]
					 	//ad9361_spi_write(clk_priv->spi,REG_TX_FORCE_VCO_TUNE_1,0x78);// Set VCO Cal Offset[3:0]
					 	//ad9361_spi_write(clk_priv->spi,REG_TX_VCO_CAL_REF,0x00);// Set VCO Cal Ref Tcf[2:0]
					 	//ad9361_spi_write(clk_priv->spi,REG_TX_VCO_VARACTOR_CTRL_1,0x0B);// Set VCO Varactor Reference[3:0]
					 	//ad9361_spi_write(clk_priv->spi,REG_TX_VCO_VARACTOR_CTRL_0,0x70);// Set VCO Varactor Ref Tcf[2:0] and VCO Varactor Offset[3:0]
					 	//ad9361_spi_write(clk_priv->spi,REG_TX_CP_CURRENT,0x8F);// Set Synth Loop Filter charge pump current (Icp)
					 	//ad9361_spi_write(clk_priv->spi,REG_TX_LOOP_FILTER_1,0xD4);// Set Synth Loop Filter C2 and C1
					 	//ad9361_spi_write(clk_priv->spi,REG_TX_LOOP_FILTER_2,0xDF);// Set Synth Loop Filter  R1 and C3
					 	//ad9361_spi_write(clk_priv->spi,REG_TX_LOOP_FILTER_3,0x09);// Set Synth Loop Filter R3

					 	//ad9361_spi_write(clk_priv->spi,REG_RFPLL_DIVIDERS,0x11);// Set Synth Loop Filter R3

					reg3 = REG_TX_INTEGER_BYTE_0;
					reg4 = REG_TX_INTEGER_BYTE_1;
					reg0 = REG_TX_FRACT_BYTE_0;
					reg1 = REG_TX_FRACT_BYTE_1;
					reg2 = REG_TX_FRACT_BYTE_2;
				...
				buf[2] = fract & 0xFF;
				buf[3] = integer >> 8;
				buf[3] = SYNTH_INTEGER_WORD(integer >> 8) |	(~SYNTH_INTEGER_WORD(~0) & ad9361_spi_read(phy->spi, reg4));
				buf[4] = integer & 0xFF;

				//ad9361_spi_writem(clk_priv->spi, reg, buf, 5);

			//	ad9361_spi_write(clk_priv->spi, reg0, 0x00);
			//	ad9361_spi_write(clk_priv->spi, reg1, 0x00);
			//	ad9361_spi_write(clk_priv->spi, reg2, 0x00);
			//	ad9361_spi_write(clk_priv->spi, reg3, 0x78);
			//	ad9361_spi_write(clk_priv->spi, reg4, 0x00);

				ad9361_spi_write(phy->spi, reg0,buf[2]);
				ad9361_spi_write(phy->spi, reg1,buf[1] );
				ad9361_spi_write(phy->spi, reg2,buf[0] );
				ad9361_spi_write(phy->spi, reg3, buf[4]);
				ad9361_spi_write(phy->spi, reg4, buf[3]);

				ad9361_spi_writef(phy->spi, REG_RFPLL_DIVIDERS, div_mask, vco_div);

				/* Load Gain Table */
				if (clk_priv->source == RX_RFPLL_INT) {
					ret = ad9361_load_gt(phy, ad9361_from_clk(rate), GT_RX1 + GT_RX2);
					if (ret < 0)
						return ret;
				}
...
/*
static int32_t ad9361_calc_rfpll_int_divder(char par,struct ad9361_rf_phy *phy,
	...
	uint32_t parent_rate)
{
	...
	uint32_t fract, integer;
	uint8_t buf[5];
	uint32_t reg0,reg1,reg2,reg3,reg4, div_mask, vco_div, profile;
	...
	if (profile) {
		...
		vco_div = ad9361_fastlock_readval(phy->spi, tx, profile, 12) & 0xF;
			...
			buf[3]=0x00;
			buf[4]=0x00;
			buf[0]=ad9361_spi_read(phy->spi, reg0);
			buf[1]=ad9361_spi_read(phy->spi, reg1);
			buf[2]=ad9361_spi_read(phy->spi, reg2);
			buf[4]=ad9361_spi_read(phy->spi, reg3);
			buf[3]=ad9361_spi_read(phy->spi, reg4);
			//ad9361_spi_read(par, reg);
			vco_div = ad9361_spi_readf(phy->spi, REG_RFPLL_DIVIDERS, div_mask);
		}
	...
	int32_t val_mV)
{
	...
	switch (dac) {
	case 1:
		ad9361_spi_write(phy->spi, REG_AUXDAC_1_WORD, val >> 2);
		ad9361_spi_write(phy->spi, REG_AUXDAC_1_CONFIG, AUXDAC_1_WORD_LSB(val) | tmp);
		phy->auxdac1_value = val_mV;
		break;
	case 2:
		ad9361_spi_write(phy->spi, REG_AUXDAC_2_WORD, val >> 2);
		ad9361_spi_write(phy->spi, REG_AUXDAC_2_CONFIG, AUXDAC_2_WORD_LSB(val) | tmp);
